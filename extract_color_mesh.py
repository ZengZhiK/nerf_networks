...
@torch.no_grad()
def batched_inference(models, embeddings, rays, N_samples, N_importance, chunk, white_back):
...
def main():
    ...
    if args.use_vertex_normal:  ## use normal vector method as suggested by the author.
        ...
        results = batched_inference([nerf_coarse, nerf_fine], embeddings,
        ...
        for idx in tqdm(range(len(dataset.image_paths))):
            ...
            results = batched_inference([nerf_fine], embeddings,
                                        torch.cat([rays_o, rays_d, near, far], 1).cuda(),
                                        args.N_samples,
                                        0,
                                        args.chunk,
